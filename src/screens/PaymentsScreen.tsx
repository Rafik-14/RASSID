...
export function PaymentsScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await getMonthlyChartData(2);
      setMonthTotal(data.total);
      const transactions = await getGlobalTransactions(2, 50);
      setTxs(transactions);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleTxPress = (tx: Transaction) => {
    ...
    showDialog({
      ...
      message: `Que voulez-vous faire avec cette opération de ${formatDAFull(Math.abs(tx.amount)).replace(' DA', '')} DA ?`,
      accentColor: c.green,
      buttons: [
        {
  ...
  const renderItem = ({ item }: { item: Transaction }) => {
    ...
    return (
      ...
      >
        <View style={[styles.txLine, { backgroundColor: c.green }]} />
        ...
        <Text style={[styles.txAmount, { color: c.green }, isVoided && { textDecorationLine: 'line-through' }]}>{formatDAFull(Math.abs(item.amount))}</Text>
      </Pressable>
    );
  };
